...
final class GetRawebRuleDocumentationMarkdown implements GetRuleDocumentationCommandInterface
{
    /**
    ...
    public function __construct(
        ...
        $rawebDataFolder = $this->projectDirectory . '/var/data/raweb-git/fr/json/';

        $rulesData = json_decode(
            file_get_contents($rawebDataFolder . 'criteres.json'),
            true,
        );
    ...
    private function getLinkListMarkdown(Rule $rule, array $ruleData): string
    {
        ...
        $links[] = sprintf(
            '- [RAWeb 1.1 - Critère %s](https://accessibilite.public.lu/fr/raweb1.1/criteres.html#crit-%s-%s)',
            $rule->getPrefix(),
            $categoryPrefix,
            $rulePrefix,
        );
